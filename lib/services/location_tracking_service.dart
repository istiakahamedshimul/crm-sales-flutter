...
Future<void> locationServiceEntry(ServiceInstance service) async {
  try {
    ...
    try {
      final initial = await Geolocator.getCurrentPosition(locationSettings: settings).timeout(const Duration(seconds: 20));
      final point = _pointFromPosition(initial);
      await _queueAndUpload(point);
      service.invoke('location', point);
    } catch (_) { /* The continuous stream below will retry. */ }
    Geolocator.getPositionStream(locationSettings: settings).listen((position) async {
      final point = _pointFromPosition(position);
      await _queueAndUpload(point);
      service.invoke('location', point);
    }, onError: (Object error) => service.invoke('tracking_error', {'message': error.toString()}));
  } catch (error) {
    service.invoke('tracking_error', {'message': error.toString()});
    await service.stopSelf();
  }
}

Map<String, dynamic> _pointFromPosition(Position position) => <String, dynamic>{
...
class LocationTrackingService {
  ...
  Future<void> configure() => _service.configure(
    ...
    androidConfiguration: AndroidConfiguration(onStart: locationServiceEntry, autoStart: false, autoStartOnBoot: true, isForegroundMode: true,
